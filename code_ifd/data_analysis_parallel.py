...
def parse_args():
    ...
    parser.add_argument("--log-file", type=str, default='./ifds.log')
...
def get_raw_data_info(dataset_root: Path, dataset_saved_root: Path, num: int = -1) -> tp.List[DataInfo]:
    ...
    data_infos = []
    origin_path = dataset_root / 'origin'
    for source_path in origin_path.glob("*.jsonl"):
...
def main():
    ...
    lock = FileLock("/cpfs01/user/yehaochen/tmp/idfs_lock.lock")
